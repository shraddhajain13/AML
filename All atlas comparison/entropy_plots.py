...
path = r"D:\Shraddha\shan_entr_all_atlas"
...
shan_entr_eFC_male_all_atlas = np.zeros([128, 11])
shan_entr_eFC_female_all_atlas = np.zeros([144, 11])



eff_size_shan_entr_eFC = []
p_val_shan_entr_eFC = []


for i in range(len(atlas)):
    print(atlas[i])
    shan_entr_eFC = pd.read_csv(os.path.join(path, "shan_entr_eFC_"+ atlas[i]) + '.csv').values[:, 1]
    ...
    shan_entr_eFC_male, shan_entr_eFC_female = categorise_male_female(shan_entr_eFC)
    shan_entr_eFC_male_all_atlas[:, i] = np.array(shan_entr_eFC_male)
    shan_entr_eFC_female_all_atlas[:, i] = np.array(shan_entr_eFC_female)
    t_value1, p_value1 = scipy.stats.ranksums(shan_entr_eFC_male, shan_entr_eFC_female)
    eff_size_shan_entr_eFC.append(pg.compute_effsize(shan_entr_eFC_male, shan_entr_eFC_female, eftype = 'hedges')) #effect size for coupling strength
...
np.savetxt(r"D:\Shraddha\Data\male_data_shan_entr_efc_br.csv", shan_entr_eFC_male_all_atlas, delimiter = ',') #this storage is for the grey plots in the background
np.savetxt(r"D:\Shraddha\Data\female_data_shan_entr_efc_br.csv", shan_entr_eFC_female_all_atlas, delimiter = ',') #this storage is for the grey plots in the background
...
#male_data_br = pd.read_csv(r"D:\Shraddha\Data\male_data_shan_entr_efc_br.csv", header = None).values #loading the ones before reg
#female_data_br = pd.read_csv(r"D:\Shraddha\Data\female_data_shan_entr_efc_br.csv", header = None).values #loading the ones before reg
...
male_plots = plt.boxplot(shan_entr_eFC_male_all_atlas, positions = np.array(range(l))*2 - 0.3)
female_plots = plt.boxplot(shan_entr_eFC_female_all_atlas, positions = np.array(range(l))*2 + 0.3)
...
effsize_pval[:, 0] = np.array(eff_size_shan_entr_eFC)
effsize_pval[:, 1] = np.array(p_val_shan_entr_eFC)

np.savetxt(r"D:\Shraddha\Data\shan_entr_efc_eff_size_p_val_br.csv", effsize_pval, delimiter = ',')
