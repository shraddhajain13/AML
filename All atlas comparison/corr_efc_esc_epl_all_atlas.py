...
for i in range(len(atlas)):
    ...
    data_efc = pd.read_csv(path_fc + atlas[i] + '.csv', header = None).values
    data_esc = pd.read_csv(path_sc + end_name_for_sc[i] + '.csv', header = None).values
    data_epl = pd.read_csv(path_pl + end_name_for_sc[i] + '.csv', header = None).values
    ...
    for j in range(len(sub_num_list_old)):
        corr_efc_esc_list.append(stats.pearsonr(data_efc[:, j], data_esc[:, j])[0])
        corr_efc_epl_list.append(stats.pearsonr(data_efc[:, j], data_epl[:, j])[0])
