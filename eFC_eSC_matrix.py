...
for filename in glob.glob(os.path.join(path1, '*17Networks_order_FSLMNI152_2mm_BOLD.csv')): 
    ...
    if subject_number in sub_num_list_old: #taking only subjects of interest (total = 272)
        ...
        for file_name in fiile: #going through each of the 4 files for one subject. this loop iterates 4 times for every subject
            ...
            four_vecs_per_subject[:,k] = upp_tri_vec_fc #np.arctanh(upp_tri_vec_fc) # collects the 4 vectors for one subject after fisher z transform
            k = k+1
        avg_four = np.mean(four_vecs_per_subject, axis = 1)
        #inv_tran_avg_four = np.tanh(avg_four)
        final_vec_fc[:,sub] =  avg_four #inv_tran_avg_four
...
#np.savetxt(r'C:\Users\shrad\OneDrive\Desktop\Juelich\Internship\Data\output_sc.csv',final_vec_sc,delimiter=",")  
#np.savetxt(r'C:\Users\shrad\OneDrive\Desktop\Juelich\Internship\Data\output_pl.csv',final_vec_pl,delimiter=",")
